...
export function MicButton({ isListening, onStart, onStop, isLoading, language, compact, isStandby }: MicButtonProps) {
  const labels = {
    su: { loading: 'Ngolah data, sakedap...', listening: 'Nuju ngupingkeun, Sobat...', idle: 'Klik upami bade nyarios', standby: 'Saurkeun: Hey ViLo' },
    ...
    <div className={`flex flex-col items-center justify-center ${compact ? 'gap-2 p-1 pointer-events-auto' : 'gap-2 py-2'}`}>
      <div className={`relative rounded-full ${isStandby ? 'bg-blue-500/10 shadow-[0_0_50px_rgba(59,130,246,0.15)] border border-blue-400/20' : 'glass shadow-[0_0_50px_rgba(16,185,129,0.15)] bg-gradient-to-tr from-emerald-500/15 via-white/5 to-teal-400/15 border border-white/10'} backdrop-blur-3xl flex items-center justify-center transition-all duration-500 ${compact ? 'p-1' : 'p-4'}`}>
        ...
        <motion.button
          ...
          disabled={isLoading}
          className={`relative ${compact ? 'h-14 w-14 lg:h-16 lg:w-16' : 'h-24 w-24 lg:h-20 lg:w-20'} rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 ${
            isListening && !isStandby
        ...
        >
          {isLoading ? (
            ...
            <Square className={`${compact ? 'w-6 h-6 lg:w-7 lg:h-7' : 'w-10 h-10 lg:w-8 lg:h-8'}`} fill="currentColor" />
          ) : (
